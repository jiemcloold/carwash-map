...
<body>
    ...
    <script>
            ...
            sortedShops.forEach((shop, index) => {
                ...
                const infoWindow = new BMap.InfoWindow(
                    \`<div style="padding:10px;">
                        ...
                        <button onclick="verifyLocation(\${shop.lat}, \${shop.lng}, '\${shop.name.replace(/'/g, "\\\\'").replace(/\n/g, ' ')}', '\${shop.address.replace(/'/g, "\\\\'").replace(/\n/g, ' ')}')" style="width:100%;margin-top:10px;padding:8px;background:#FF9800;color:white;border:none;border-radius:6px;font-size:13px;font-weight:500;">🔍 核实位置</button>
        ...
        // 核实商家位置
        function verifyLocation(lat, lng, name, address) {
            // 关闭所有信息窗口
            map.closeInfoWindow();
            ...
            // 移动地图中心到该商家，并放大到18级（街道级别）
            const point = new BMap.Point(lng, lat);
            map.centerAndZoom(point, 18);

            // 在顶部信息栏显示商家详情
            const infoHtml = \`
                <div style="background:#FFF3E0;padding:10px;border-radius:6px;border:2px solid #FF9800;">
        ...
        function navigateTo(lat, lng, name, address, phone, type) {
            ...
            if (type === 'amap') {
                // 高德地图 - 传递详细信息
                window.location.href = \`https://uri.amap.com/navigation?to=\${lng},\${lat}&toname=\${encodeURIComponent(description)}&mode=car&policy=1&src=carwash\`;
            } else {
                // 百度地图 - 传递详细信息
                window.location.href = \`baidumap://map/direction?destination=\${lat},\${lng}&destination_name=\${encodeURIComponent(description)}&coord_type=bd09ll&mode=driving&src=carwash\`;
            }
        }
