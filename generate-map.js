...
<body>
    ...
    <script>
                ...
                const infoWindow = new BMap.InfoWindow(
                    \`<div style="padding:10px;">
                        ...
                        <div style="margin-top:10px;display:flex;gap:8px;">
                            <button onclick="navigateTo(\${shop.lat}, \${shop.lng}, '\${shop.name.replace(/'/g, "\\\\'")}', 'amap')" style="flex:1;padding:8px;background:#00A870;color:white;border:none;border-radius:6px;font-size:13px;font-weight:500;">🧭 高德导航</button>
                            <button onclick="navigateTo(\${shop.lat}, \${shop.lng}, '\${shop.name.replace(/'/g, "\\\\'")}', 'baidu')" style="flex:1;padding:8px;background:#1989fa;color:white;border:none;border-radius:6px;font-size:13px;font-weight:500;">🧭 百度导航</button>
                        </div>
        ...
        // 导航到指定商家
        function navigateTo(lat, lng, name, type) {
            if (type === 'amap') {
                // 高德地图
                window.location.href = \`https://uri.amap.com/navigation?to=\${lng},\${lat}&toname=\${encodeURIComponent(name)}&mode=car&policy=1&src=carwash\`;
            } else {
                // 百度地图
                window.location.href = \`baidumap://map/direction?destination=\${lat},\${lng}&destination_name=\${encodeURIComponent(name)}&coord_type=gcj02&mode=driving&src=carwash\`;
            }
        }
